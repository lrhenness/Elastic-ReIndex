...
def check_task(task): #Check on the task:
    get = requests.get("https://" + host + ":" + port + "/_tasks/" + task, verify=False).json() #GET task information and convert JSON response to Python dictionary
...
def reindex():
    ...
    post = requests.post("https://" + host + ":" + port + "/_reindex?wait_for_completion=false", data=body)
    for line in list:
        ...
        resp = requests.post("https://" + host + ":" + port + "/_reindex?wait_for_completion=false", data=body, verify=False).json()
